...
filenames = df_inputs.FileName.tolist()
df_inputs.drop(columns=["FileName"],inplace=True)
...
url_base = base_url+tool_name                        # Add the tool name to the base url


for i in tqdm(range(len(df_inputs))):                 # Loop through all of the lines on the input spreadsheet
    url = url_base
